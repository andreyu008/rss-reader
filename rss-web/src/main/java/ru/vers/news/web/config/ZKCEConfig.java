...
public class ZKCEConfig {
  ...
  private static String ZUL_VIEW_RESOLVER_PREFIX =
      UPDATE_URI + ClassWebResource.PATH_PREFIX + "/zul/";
  ...
  public ViewResolver zulViewResolver() {
    InternalResourceViewResolver resolver =
        new InternalResourceViewResolver(ZUL_VIEW_RESOLVER_PREFIX, ZUL_VIEW_RESOLVER_SUFFIX);
  ...
  @Bean
  @ConditionalOnMissingClass("org.zkoss.zats.mimic.Zats") // only allow custom update URI outside
                                                          // Zats testcases.
  ...
  /**
   * With Zats the listener needs to be configured in web.xml.(custom update URI isn't supported by
   * Zats anyway). Zats runs with its own embedded Jetty.
   */
  @Bean
      ...
      public void contextInitialized(ServletContextEvent sce) {
        ...
        if (WebManager.getWebManagerIfAny(ctx) == null) {
          ...
          throw new IllegalStateException(
              "ZK WebManager already exists. Cannot initialize via Spring Boot configuration.");
        }
      }
